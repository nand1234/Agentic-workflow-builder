...
1. **Test user behaviour, not implementation** — assert what the user sees and can do, not which function was called.
2. **One assertion per concept** — each `expect()` should test exactly one thing.
3. **Prefer semantic selectors** — `getByRole` > `getByLabel` > `getByTestId` > CSS. Never `.querySelector('.some-class')`.
4. **Explicit waits only** — `await expect(locator).toBeVisible()`, never `page.waitForTimeout(2000)`.
5. **Isolated tests** — each test sets up its own state, cleans up after itself.
6. **Page Object Model** — all selectors and actions in page objects, not in test files.
